...
firFox = webdriver.Firefox(executable_path=r'C:\Users\DELL\Desktop\typhoon_predict\web_back\tool\geckodriver.exe')
...
def get_message(typoon_names):
    ...
    for i in range(1,len(trs)+1):
        ...
        try:
            ...
            print("null of message  "+ str(typoon_name))
...
def name_find(num):
    ...
    trs = tbody_.find_elements_by_tag_name("tr")
    with open(f"./typhoon_file/typhoon_{num}.csv","w",encoding="utf-8") as f:
        for tr in trs:
            tr = tr.text.replace(" ",",")
            f.write(tr+"\n")
