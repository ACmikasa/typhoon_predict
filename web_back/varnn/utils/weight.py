...
class GaussianWeight(object):
    ...
    def sample(self):
        ...
        sample = self.mu + (self.sigma*epsilon).to(self.mu.device)
        return sample 
    
    def log_prob(self, x):
        """Log PDF
        """
        return (-math.log(math.sqrt(2*math.pi)) - torch.log(self.sigma.to(x.device)) - (((x - self.mu)**2)/(2 * self.sigma**2).to(x.device))).sum()
